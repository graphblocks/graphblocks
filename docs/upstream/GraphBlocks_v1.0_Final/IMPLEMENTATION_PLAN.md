...
- Callback rejection paths now emit durable `ExternalCallbackRejected` metadata events for stale
  attempts, run/node identity mismatches, schema mismatches, and payload-limit failures without
...
- The Rust callback retry policy constructor normalizes zero delay settings to a positive delay
  floor, preventing callback retry scheduling from devolving into a tight retry loop even when
  caller-side validation misses an invalid duration.
...
- `graphblocks-callbacks` now maps webhook receiver HTTP responses into delivery decisions:
  ...
  failure, including `Retry-After` parsing for retry scheduling. Absolute `Retry-After` values that
  are already stale at receipt time are ignored so bounded retry policy remains authoritative.
