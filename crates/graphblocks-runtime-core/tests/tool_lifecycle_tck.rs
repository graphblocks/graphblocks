...
fn run_case(case: &Value) -> Result<(), String> {
    ...
    match required_str(case, "kind")? {
        ...
        "admission_tool_name_mismatch" => run_tool_name_mismatch_case(case_name, case),
        "admission_arguments_digest_mismatch" => {
            run_arguments_digest_mismatch_case(case_name, case)
        }
...
fn run_missing_schema_case(case_name: &str, case: &Value) -> Result<(), String> {
    let expected = expected(case, case_name)?;
    let schema_id = required_str(case, "schemaId")?;
    let tool_name = required_str(case, "toolName")?;
    let resolved_tool = resolved_process_tool(tool_name, schema_id)?;
    let schemas = ToolSchemaRegistry::default();
    let call = tool_call_from_arguments(
        tool_name,
        &resolved_tool.resolved_tool_id,
        case.get("arguments")
...
fn run_tool_name_mismatch_case(case_name: &str, case: &Value) -> Result<(), String> {
    let expected = expected(case, case_name)?;
    let schema_id = required_str(case, "schemaId")?;
    let tool_name = required_str(case, "toolName")?;
    let resolved_tool = resolved_process_tool(tool_name, schema_id)?;
    let schemas = process_schema_registry(schema_id)?;
    let call = tool_call_from_arguments(
        required_str(case, "callToolName")?,
        &resolved_tool.resolved_tool_id,
        case.get("arguments")
            .cloned()
            .ok_or_else(|| format!("tool-lifecycle TCK case {case_name} missing arguments"))?,
    ...
    assert_eq!(
        matches!(result, Err(ToolAdmissionError::ToolNameMismatch { .. })),
        required_bool(expected, "toolNameMismatchBeforeSchema")?,
        "{case_name}",
    );
    assert!(
        error_text.contains(required_map_str(expected, "errorContains")?),
...
fn run_arguments_digest_mismatch_case(case_name: &str, case: &Value) -> Result<(), String> {
    let expected = expected(case, case_name)?;
    let schema_id = required_str(case, "schemaId")?;
    let tool_name = required_str(case, "toolName")?;
    let resolved_tool = resolved_process_tool(tool_name, schema_id)?;
    ...
    assert_eq!(
        ...
        required_bool(expected, "argumentsDigestRejectedBeforeSchema")?,
        "{case_name}",
    );
    assert!(
        error_text.contains(required_map_str(expected, "errorContains")?),
...
fn run_missing_approval_case(case_name: &str, case: &Value) -> Result<(), String> {
    let expected = expected(case, case_name)?;
    let schema_id = required_str(case, "schemaId")?;
    let tool_name = required_str(case, "toolName")?;
    let resolved_tool = resolved_process_tool(tool_name, schema_id)?;
...
fn admission_error_text(error: &ToolAdmissionError) -> &'static str {
    match error {
        ...
        ToolAdmissionError::ArgumentsDigestMismatch { .. } => "digest",
        ...
        ToolAdmissionError::InputSchemaMissing { .. } => "schema",
        ...
        ToolAdmissionError::ToolNameMismatch { .. } => "name",
        _ => "admission failed",
    }
}
