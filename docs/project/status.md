...
| Durability and recovery (`durability`) | `core-c1-runtime-authority-and-correctness-blocked-production-preview` | `core-c1-local-runtime-authority`; blocks target release | `REL-AUDIT-REMEDIATION`, `REL-NORMATIVE-AUTHORITY`, `REL-RUNTIME-CORRECTNESS` | The four remaining C1 requirements, six remaining exact-differential suites, and restart-independent local correctness block 1.0; the installed native runtime API, raw application-event fixture construction, and materialized event admission slice are frozen, while C4/X3 production durability is governed by a separate nonblocking preview-promotion gate. |
