...
| Local runtime, cancellation, tools, budget core | Implemented reference interpreter and exact oracle for the bundled stable runtime, retry, and sequence suites | Active authority for the installed stable local scheduler/journal, retry, and bounded-sequence slices; typed ports, outcomes, cancellation, local flow, and three exact-differential suites still block complete C1 authority |
...
`exact-native-reference`. The `application-events`, `retry`, and `sequence`
suites are also `exact-native-reference`; three other C1 suites remain
`reference-only`.
