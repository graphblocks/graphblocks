...
- Callback rejection paths now emit durable `ExternalCallbackRejected` metadata events for stale
  attempts, run/node identity mismatches, schema mismatches, payload-limit failures, and callbacks
  that target operations not currently waiting for a callback, without journaling rejected payload
  bodies; SQLite persistence covers these rejection events across reopen.
