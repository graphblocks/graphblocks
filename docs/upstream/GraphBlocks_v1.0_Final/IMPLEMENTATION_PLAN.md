...
- Durable background-run shared TCK cases now require an object `initialResponse` carrying
  ...
  runners, and require the event-stream handle to encode the returned run id, preventing truthy
  placeholder values, missing websocket or cancellation links, or mismatched run links from
