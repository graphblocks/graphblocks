...
`GB-PERF-008` is resolved by a digest-bound deterministic microbenchmark gate
on the canonical Linux/Python 3.11 lane. Decimal canonicalization runs through
16,000 values, append-friendly journals through 64,000 records, the Python
compiler through 800 nodes, and retained server state through 20 completed
runs. Every size has an absolute cap and every first-to-last pair has a
normalized growth cap; elapsed cases use warmup plus three median observations,
...
`python-reference` identity. Standalone canonical serialization/hash, SchemaId
identity, resource validation, and resource migration now route through
fail-closed native facades, while explicit Python reference entry points remain
...
three exact-differential suites, restart-independent local correctness, and
...
`application-events`, `retry`, `sequence`, `tool-execution`, and
`tool-lifecycle` suites also use installed Rust executors; the one remaining C1
suite stays `reference-only`.
This preserves
...
the closed result flags with the Python oracle. Its evidence is assigned to
`rust-tool-lifecycle-exact-differential` and the same exact runtime wheel.
...
verified.
The repository-enforced notice-only Rust surface, fail-closed npm import, and
package-content smoke gates resolve `GB-ARCH-018`; the registry publication
blockers remain external artifact-state work and do not reopen the source
packaging ambiguity.
