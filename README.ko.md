...
GraphBlocks 1.0은 안정화된 C0 스키마/컴파일러와 C1 로컬 런타임 코어를 제공합니다.
호환성은 패키지나 디렉터리의 존재 여부가 아니라 적합성 프로필과 실행 가능한 증거를
기준으로 선언됩니다. Graph 컴파일러의 규범적 구현은 Rust이며, 상위 프로필과 더 넓은
네이티브 런타임 표면은 릴리스 매트릭스에 따라 프리뷰로 남습니다.
