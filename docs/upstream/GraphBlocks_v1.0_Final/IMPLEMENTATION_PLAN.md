...
- Durable background-run shared TCK cases now require nonblank event `eventId`/`event_id`,
  ...
  increasing positive integer `sequence` fields in both Rust and Python runners, and reject
  duplicate event ids or replay cursors before proving cursor replay, preventing anonymous,
  duplicate, untyped, cursorless, untimestamped, or unordered events from satisfying replay
  assertions.
