...
- `graphblocks-server` now exposes the framework-neutral `POST /runs/{run_id}/detach`
  ...
  requests from the same client are idempotent and return the first detach record.
...
- Durable callback projection fixtures now carry and validate `subscriptionId`, cursor, and
  ...
  cursor, non-integer `sequence`, and non-integer `attempt` evidence, aligning callback
  conformance with the protocol's at-least-once delivery envelope.
...
- Durable background-run shared TCK cases now require an object `initialResponse` carrying
  nonblank `runId`/`run_id` and `initialCursor`/`initial_cursor` values in both Rust and Python
  runners, preventing truthy placeholder values from satisfying durable response and replay-handle
  semantics.
...
- Durable background-run shared TCK cases now require nonblank event `eventId`/`event_id`,
  nonblank `cursor`, ISO-valid `occurredAt`/`occurred_at`, and strictly increasing non-boolean
  non-negative integer `sequence` fields in both Rust and Python runners before proving cursor
  replay, preventing anonymous, cursorless, untimestamped, or unordered events from satisfying
  replay assertions.
...
- Durable external-operation reconciliation shared TCK cases now require a nonblank
  `operation.operationId` in both Rust and Python runners before accepting late side-effect,
