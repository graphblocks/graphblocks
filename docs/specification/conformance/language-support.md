...
| Local runtime, cancellation, tools, budget core | Implemented reference interpreter and exact oracle for the bundled stable runtime and retry suites | Active authority for the installed stable local scheduler/journal and retry slices; typed ports, outcomes, cancellation, local flow, and four exact-differential suites still block complete C1 authority |
...
`exact-native-reference`. The `application-events` and `retry` suites are also
`exact-native-reference`; four other C1 suites remain `reference-only`.
