...
#[test]
fn sqlite_budget_ledger_release_with_expired_permit_rejects_without_mutating()
-> Result<(), BudgetError> {
    let mut ledger = SqliteBudgetLedger::open_in_memory()?;
    ledger.allocate("budget-1", "tenant:acme", [tokens(100)], "policy-1", None)?;
    let reservation = ledger.reserve(
    ...
    let error = ledger
        ...
        .expect_err("expired permit cannot release a SQLite reservation");
...
fn budget_ledger_release_with_expired_permit_rejects_without_mutating() -> Result<(), BudgetError> {
    ...
    let error = ledger
        ...
        .expect_err("expired permit cannot release a reservation");
