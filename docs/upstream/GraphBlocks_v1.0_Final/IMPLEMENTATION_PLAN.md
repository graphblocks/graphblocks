...
- Shared durable async callback resume TCK fixtures now validate supplied operation envelopes,
  requiring nonblank `operationId`, `runId`, `nodeId`, and `attemptId` evidence, so
  journal-before-resume conformance cannot be proven from an anonymous `AsyncOperation` reference.
