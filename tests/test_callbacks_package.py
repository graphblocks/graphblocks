...
def test_callback_webhook_verification_rejects_case_duplicate_headers() -> None:
    envelope = CallbackEnvelope(
        delivery_id="del_001",
        subscription_id="sub_001",
        event_id="evt_1042",
    ...
    headers = webhook_headers_hmac_sha256(envelope, b"callback-secret")
    ambiguous_headers = dict(headers)
    ambiguous_headers["graphblocks-signature"] = headers["GraphBlocks-Signature"]
...
def test_callback_webhook_hmac_keyring_rejects_empty_keyring() -> None:
    envelope = CallbackEnvelope(
        delivery_id="del_001",
        subscription_id="sub_001",
        event_id="evt_1042",
    ...
    _assert_raises_value_error(
        "secrets_by_key_id must contain at least one key",
        lambda: verify_webhook_headers_hmac_sha256_keyring(
            envelope,
            headers,
            {},
...
def test_callback_webhook_hmac_keyring_validates_entries_before_headers() -> None:
    envelope = CallbackEnvelope(
        delivery_id="del_001",
        subscription_id="sub_001",
        event_id="evt_1042",
    ...
    _assert_raises_value_error(
        "secret values must be non-empty bytes",
        lambda: verify_webhook_headers_hmac_sha256_keyring(
            envelope,
...
def test_callback_dead_letter_record_rejects_inconsistent_delivery_state() -> None:
    ...
    for attempt_history in ((1, 3), (1, 2, 2, 3), (2, 3)):
        _assert_raises_value_error(
            "dead-letter record attempt_history must be consecutive from attempt 1",
            lambda attempt_history=attempt_history: CallbackDeadLetterRecord(
                delivery=CallbackDeliveryProjection(
                    delivery_id=f"del_history_{'_'.join(str(item) for item in attempt_history)}",
                    subscription_id="sub_001",
                    event_id="evt_1042",
                    run_id="run_coding_001",
                    sequence=1042,
                    cursor="evt_1042",
                    attempt=3,
                    idempotency_key=f"sub_001:evt_1042:{'_'.join(str(item) for item in attempt_history)}",
                    status="dead_lettered",
                    last_error="receiver 503",
                ),
...
def test_external_callback_receipt_rejects_unauthenticated_verifier() -> None:
    envelope = CallbackEnvelope(
        delivery_id="cb_001",
        subscription_id="sub_001",
        event_id="evt_callback_001",
    ...
    _assert_raises_value_error(
        "verified_by must identify an authenticated verifier",
...
def test_external_callback_receipt_rejects_non_external_callback_event_type() -> None:
    envelope = CallbackEnvelope(
        delivery_id="cb_001",
        subscription_id="sub_001",
        event_id="evt_callback_001",
    ...
    _assert_raises_value_error(
        "envelope type must be ExternalCallbackReceived",
        lambda: record_external_callback_receipt(
            envelope,
            projection,
            operation_id="op_ci_001",
...
def test_external_callback_receipt_rejects_envelope_operation_mismatch() -> None:
    envelope = CallbackEnvelope(
        delivery_id="cb_001",
        subscription_id="sub_001",
        event_id="evt_callback_001",
        ...
        occurred_at="2026-07-02T00:00:00Z",
        delivered_at="2026-07-02T00:00:01Z",
        operation_id="op_ci_envelope",
    )
    projection = project_callback_payload(envelope.payload, max_inline_bytes=256)

    _assert_raises_value_error(
        "operation_id must match the envelope",
        lambda: record_external_callback_receipt(
            envelope,
            projection,
            operation_id="op_ci_other",
            node_id="waitCI",
            attempt_id="attempt_001",
            verified_by="hmac-sha256:key-current",
            policy_snapshot_id="policy_001",
...
def test_external_callback_receipt_validates_received_timestamp() -> None:
    ...
    _assert_raises_value_error(
        "received_at must be an ISO-8601 datetime",
        lambda: ExternalCallbackReceipt(
            callback_id="cb_001",
            operation_id="op_ci_001",
            run_id="run_coding_001",
            ...
            idempotency_key="op_ci_001:attempt_001:provider_001",
            payload_projection=projection,
            payload_digest=projection.payload_digest or "",
            received_at="eventually",
            verified_by="hmac-sha256:key-current",
            policy_snapshot_id="policy_001",
        ),
    )
...
def test_callback_endpoint_ref_requires_complete_resume_identity() -> None:
    ...
    _assert_raises_value_error(
        "url must not contain embedded userinfo",
        lambda: CallbackEndpointRef(
            endpoint_id="cbep_ci_001",
            url="https://token@graphblocks.example.com/v1/callbacks/op_ci_001",
            accepted_schema="schemas/CICallback@1",
            auth=CallbackEndpointAuth(kind="hmac", secret_ref="secret://callbacks/ci"),
            operation_id="op_ci_001",
            run_id="run_coding_001",
...
def test_callback_resume_admission_rejects_receipt_at_endpoint_expiration() -> None:
    ...
    assert decision.status == "expired"
    assert decision.can_resume is False
    assert decision.reason == "callback_received_after_endpoint_expiration"
...
def test_callback_resume_admission_rejects_provider_operation_mismatch() -> None:
    endpoint = CallbackEndpointRef(
        endpoint_id="cbep_ci_001",
        url="https://graphblocks.example.com/v1/callbacks/op_ci_001",
        accepted_schema="schemas/CICallback@1",
        ...
        tenant_id="tenant_001",
        expires_at="2026-07-02T00:30:00Z",
        provider_operation_id="gh_current",
    )
    envelope = CallbackEnvelope(
        delivery_id="cb_001",
        subscription_id="sub_001",
    ...
    receipt = record_external_callback_receipt(
        ...
        policy_snapshot_id="policy_001",
        received_at="2026-07-02T00:00:02Z",
        provider_operation_id="gh_stale",
    )
    ...
    assert decision.status == "stale"
    assert decision.can_resume is False
    assert decision.reason == "callback_provider_operation_mismatch"
